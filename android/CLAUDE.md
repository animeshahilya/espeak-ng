...
- **TtsSettingsActivity** — Preferences UI, declared in `res/xml/preferences.xml` (list choices in `res/values/arrays.xml`) and inflated once the engine has loaded; `buildPreferences()` fills in the engine-dependent rows and drops the Wear-hidden ones. `UserDictionaryScreen` holds the user dictionary editor; also the `CONFIGURE_ENGINE` target and, on Wear, the launcher entry point. Voice-picker labels are localized via `getVoiceLabel()`: the system-language name first (`Locale.getDisplayName()`), English name in parentheses for disambiguation
